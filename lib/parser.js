// The code in this file is used to map a string like "a+b*" to a lisp-style expression
// This mapping is done using a parser combinator library called Parsimmon. The expressions are 
// represented using javascript Arrays.

// Each expression tree starts with a root node.
var exprs = require('./exprs.js');
var root = exprs.shape.oneArg('root');

// Parts of a regular expression are predicates over a single character, like "a" or "[abc]". 
// Each predicate contains a little piece of code, itself represented as a lisp-style expression. 
var predicate = function(x) { return ['predicate', ['quote', x]]; };
...
// The familiar regular expression character classes. These are represented as zero-argument 
// expressions like (wildcard ()).
...
// Boilerplate for defining parsers involving bslashes, e.g. for special character classes and
// escaped characters.
var bslashed = function(string, result) {
  var pattern = '\\' + string;
  return Parsimmon.string(pattern).result(result).desc(pattern);
};
var escape = function(c) { return bslashed(c, charEquals(quote(c))) };
...
var character = Parsimmon.regex(/[^\\*+?()\[\].|]/).map(quote).map(charEquals).desc('character');

// Special characters that require an escaping bslash.
var escChars = ['(', ')', '[', ']', '*', '+', '\\', '.', '|'];
var escaped = Parsimmon.alt.apply(null, escChars.map(escape));

// Some special character classes that are parsed from a bslashed character to a symbol.
...
var charClassChar = customClassChar.map(quote).map(charEquals);
...
var predicate = Parsimmon.alt(escaped, charClasses, wildcard, character).map(predicate);

// An atom is any regular expression that can be modified by a repetition operator.
var atom = Parsimmon.alt(parenthesized(regex), predicate).desc('atom');
